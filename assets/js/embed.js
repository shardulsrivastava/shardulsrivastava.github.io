(function() {
    ...
    var addDefaultContent = function(element, options) {
        ...
        if (defaultContent !== "") {
            var cleanedConent = defaultContent.trim().replace(/\uFFFD/g, "").replace(/“/g, '"').replace(/”/g, '"');
    ...
    var createOptions = function(ui, element, originalHost) {
        ...
        var options = "/?v=2&embed=true&ui=" + ui + "&host=" + originalHost + "&url=" + encodeURIComponent(window.location.href) + "&target=" + target + "&nonce=" + window.katacoda.nonce;
        var items = ["layout", "port", "lang", "token", "userid", "filename", "command", "runinbackground", "prompt", "color", "secondary", "background", "hideintro", "hidefinish", "hidetitle", "hidesidebar", "hideprogress", "font", "fontheader", "startscenariobuttontext", "ctaurl", "ctatext", "externalcss", "hideiframebuttons"];
    ...
    var processInline = function(host, element) {
        ...
        var minHeight = element.style.height || (layout === "editor" ? "200px" : "250px");
    ...
    var writeToTerminal = function(cmd) {
        ...
        if (target.length === 0) target = document.querySelectorAll("[data-katacoda-id]");
        if (target.length === 0) {
    ...
    var createnonce = function(cb) {
        ...
        oReq.open("GET", protocol + host + "/api/embed/nonce?protocol=" + window.location.protocol + "&host=" + window.location.host);
    ...
    var logout = function() {
        ...
        oReq.open("GET", window.location.protocol + "//" + host + "/logout?protocol=" + window.location.protocol + "&host=" + window.location.host);
        oReq.setRequestHeader("X-Requested-With", "XMLHttpRequest");
        oReq.send();
        return
    };
